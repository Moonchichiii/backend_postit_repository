...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    def validate_profile_image(self, value):
        ...
        if value.size > max_size_mb * 1024 * 1024:
            raise serializers.ValidationError(f'Profile image size larger than {max_size_mb}MB.')

        if value.image.height > max_height_px:
            raise serializers.ValidationError(f'Profile image height larger than {max_height_px}px')

        if value.image.width > max_width_px:
            raise serializers.ValidationError(f'Profile image width larger than {max_width_px}px')
